...
app.whenReady().then(() => {
  ...
  // --- NEW: Register with Launch Services on first launch ---
  if (app.isPackaged && process.platform === 'darwin') {
    const { exec } = require('child_process');
    const appPath = app.getPath('exe'); // Get the path to the executable itself
    ...
    // Using exec instead of spawn for simplicity with lsregister
    exec(`"${lsregisterPath}" -f "${appPath}"`, (error, stdout, stderr) => {
      if (error) {
        console.error(`lsregister exec error: ${error}`);
      }
      if (stderr) {
        console.error(`lsregister stderr: ${stderr}`);
      }
      console.log(`lsregister stdout: ${stdout}`);

      // --- NEW: Relaunch Finder ---
      exec(`osascript -e 'tell application "Finder" to quit' -e 'delay 0.1' -e 'tell application "Finder" to launch'`, (finderError, finderStdout, finderStderr) => {
        if (finderError) {
          console.error(`Finder relaunch error: ${finderError}`);
        }
        if (finderStderr) {
          console.error(`Finder relaunch stderr: ${finderStderr}`);
        }
        console.log(`Finder relaunch stdout: ${finderStdout}`);
      });
      // --- END NEW ---
    });
  }
